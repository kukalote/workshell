...
struct Todo * explodeLine(char * line, struct Todo *todo);
...
char * getTodoPath( void ){
    ...
    home = getenv("HOME");
    strcat(home, "/");
    strcat(home, "workshell/cshell/todo.txt");
    return home;
}
...
struct Todo * explodeLine(char * line, struct Todo *todo) {
    ...
    date = strtok(line, s);
    if( date != ((void *)0) ) {
        time = strtok(((void *)0), s);
        tst = Str2TimeStamp(date, time);
        todo->time_stamp = 1457236985;
        if(tst==todo->time_stamp) {
            printf("equel\n");
            todo->time_stamp = Str2TimeStamp(date, time);
        }


        todo->message = strtok(((void *)0), s);
        todo->address = strtok(((void *)0), s);
        todo->note = strtok(((void *)0), s);
...
unsigned long Str2TimeStamp(char * date, char * time) {
    ...
    month = atoi(strtok(((void *)0), date_s));
    day = atoi(strtok(((void *)0), date_s));

    hour = atoi(strtok(time, time_s));
    min = atoi(strtok(((void *)0), time_s));
    sec = atoi(strtok(((void *)0), time_s));
